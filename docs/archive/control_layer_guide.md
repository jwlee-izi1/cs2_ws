...
> **ARCHIVED 2026-05-15** — this is the LEGACY control layer (`ros_gz_crazyflie` +
> Gazebo's `MulticopterVelocityControl` plugin). It has been superseded by **CrazySim**
> (full Crazyflie firmware as SITL in Docker, bridged to Gazebo). For the current
> control plane see [../../CRAZYSIM_MIGRATION.md §1](../../CRAZYSIM_MIGRATION.md).
> The legacy `payload_hover.launch.py` path still exists in `cf_payload_world` for
> backward comparison; see [../payload.md](../payload.md).
