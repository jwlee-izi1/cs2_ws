...
> **ARCHIVED 2026-05-15** — this describes the SURVEILLANCE variant of Fed-DCSA
> (charging stations + standby drone swapping in a 4-sector arena). That project
> was abandoned. The `fed_dcsa/` package is now used for two different Fed-DCSA
> problems: payload tilt correction (`payload_optimizer_node`) and the CDC 2026
> radial coverage demo (`radial_coverage_node`). For the active project see
> [../federated_coverage.md](../federated_coverage.md).
>
> The legacy code referenced here (`coordinator_node.py`, `algorithm.py`,
> `fed_dcsa_experiment.launch.py`) may or may not still build — treat as historical
> reference only.
